...
export default function OrderPage({ user, venueId }) {
  ...
  useEffect(() => {
    axios.get(`${ORDER_API}/api/menu/${venueId}`)
      .then(r => setMenu(r.data.items || []))
      .catch(() => toast.error('Could not load menu'))
      .finally(() => setLoading(false));
  }, [venueId]);

  const add = (itemId) => setCart(c => ({ ...c, [itemId]: (c[itemId] || 0) + 1 }));
  const sub = (itemId) => setCart(c => {
    const n = (c[itemId] || 0) - 1;
    if (n <= 0) { const nc = { ...c }; delete nc[itemId]; return nc; }
    return { ...c, [itemId]: n };
  });
  ...
  const placeOrder = async () => {
    if (!cartItems.length) { toast('Add something to your cart first!'); return; }
    setPlacing(true);
    try {
      const items = cartItems.map(m => ({
        itemId: m.id, name: m.name, quantity: cart[m.id], price: m.price,
      }));
      const res = await axios.post(`${ORDER_API}/api/orders`, {
        userId: user.uid,
        venueId,
      ...
      {lastOrder && (
        <div className="mx-4 mt-4 bg-green-50 border border-green-200 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-green-600 mt-1">Order ID: {lastOrder.orderId?.slice(0, 8)}</p>
          </div>
        </div>
      )}
      ...
      <div className="px-4 mt-4 space-y-6">
        {loading ? (
          ...
          Object.entries(groupedMenu).map(([cat, items]) => (
            <div key={cat}>
              <h3 className="font-semibold text-gray-500 text-xs uppercase tracking-wider mb-2">{cat}</h3>
                ...
                {items.map(item => (
                  <div key={item.id} className="bg-white rounded-xl shadow-sm p-3 flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-semibold text-orange-600 mt-0.5">₹{item.price}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      {cart[item.id] > 0 ? (
                        <>
                          ...
                          <span className="w-4 text-center font-medium text-sm">{cart[item.id]}</span>
        ...
        <div className="fixed bottom-16 left-0 right-0 px-4">
          <div className="max-w-md mx-auto bg-orange-500 text-white rounded-2xl p-4 shadow-lg flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold">{cartItems.length} item(s) · ₹{total.toFixed(2)}</span>
            </div>
